...
def appendData(store, hdf_name, df_to_append):
    if hdf_name in store:
        existing_df = store.get(hdf_name)

        df_to_append.insert(0, existing_df)
        new_df = pd.concat(df_to_append, ignore_index=True, sort=False)
    else:
        new_df = pd.concat(df_to_append, ignore_index=True, sort=False)

    if hdf_name == "proto_moduleset":
        store.put(hdf_name, new_df, format="table", data_columns=True, min_itemsize=50000)
    else:
        store.put(hdf_name, new_df, format="table", data_columns=True, min_itemsize=100)


def parseFile(store, filename, **options):
    ...
    file_group = store.create_group(file_groupname)

    # add metadata for the file group
    # date finished
    file_group.attrs["date_finished"] = str(getFileDate(filename))
    ...
    for module in modules:
        ...
        for test_ind, test in enumerate(tests):
            ...
            row = (test, passed, num_runs_ok, num_runs, num_cmps_ok, num_cmps, start_time, elapsed_time, timed_out)
            test_dataset_arr[test_ind] = row

        # create the dataset
        module_dataset = file_group.create_dataset(module, (total_module_tests,), data=test_dataset_arr)
...
def handle(store, *args, **options):
    ...
    regtest_df_to_append = []
    regtestset_df_to_append = []
    subregtest_df_to_append = []
    proto_moduleset_df_to_append = []
    module_df_to_append = []
    ...
    for mod in modules:
        ...
        regtestset_df_to_append.append(df_new_row)

        curr_regtestset_id = len(regtestset_df_to_append) + len(store["regtestset"]) - 1
    ...
    for module in modules:  # can use this to also populate module_set (naming it mod_set() )
        ...
        curr_module_id = len(store["module"]) - 1
        if module_data["name"] not in store["module"]["name"].values:
            df_new_row = pd.DataFrame([module_data], columns=columns)
            module_df_to_append.append(df_new_row)
            curr_module_id = len(module_df_to_append) + len(store["module"]) - 1
        ...
        for test in sorted(regdata[module]["tests"]):
            ...
            regtest_df_to_append.append(df_new_row)
            curr_regtest_id = len(regtest_df_to_append) + len(store["regtest"]) - 1
            ...
            try:
                for sub_tests in regdata[module]["tests"][test]["sub_tests"]:
                    ...
                    subregtest_df_to_append.append(df_new_row)
                ...
                for sub_cmps in regdata[module]["tests"][test]["sub_cmps"]:
                    ...
                    sub_cmp_return_code = str(regdata[module]["tests"][test]["sub_cmps"][sub_cmp_name]["return_code"])
                    sub_cmp_type = "C"
        ...
        proto_moduleset_df_to_append.append(df_new_row)
        ...
        print("\n\n\n\n\n\n\n")
        """
    # appendData(store, "regtest", df_regtest_curr)
    # appendData(store, "regtestset", df_regtestset_curr)
    # appendData("regtestsetid", df_regtestset_curr)
    # appendData(store, "subregtest", df_subregtest_curr)
    # appendData(store, "proto_moduleset", df_proto_moduleset_curr)

    st = default_timer()
    appendData(store, "regtest", regtest_df_to_append)
    appendData(store, "regtestset", regtestset_df_to_append)
    appendData(store, "subregtest", subregtest_df_to_append)
    appendData(store, "proto_moduleset", proto_moduleset_df_to_append)
    print(f"Append data time: {default_timer() - st} seconds")

    # TODO: Figure whatever df_module_curr is
    # try:
    #     df_module_curr
    # except NameError:
    #     print("module check")
    # else:
    #     appendData(store, "module", df_module_curr)
    if len(module_df_to_append) == 0:
        print("module check")
    else:
        appendData(store, "module", module_df_to_append)
...
def main():
    ...
    # with pd.HDFStore(h5_file_name) as store:
    ...
    # Add 5 tests
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-05_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-06_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-07_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-08_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-09_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-10_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-11_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-12_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-13_05", rtype="hourly", suite="suiteName")
    # handle(store, file="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-14_05", rtype="hourly", suite="suiteName")
    # parseFile(store, "/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-05_05", rtype="hourly", suite="suiteName")

    regtest_dir = "/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38"
    data_file_names = sorted(
        [os.path.join(regtest_dir, x) for x in os.listdir(regtest_dir) if "regtest.data" in x], reverse=True
    )

    with h5py.File("hierarchical_test.h5", "w") as store:
        for i, filename in enumerate(tqdm(data_file_names)):
            parseFile(store, filename=filename)

            if i > 1000:
                break

        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-05_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-06_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-07_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-08_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-09_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-10_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-11_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-12_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-13_05")
        # parseFile(store, filename="/home/dlab3/repo/PKGBUILDS/ROAMSDshellPkg/ROAMSDshellPkg-Hourly-FC38/regtest.data-2023-06-11-14_05")
